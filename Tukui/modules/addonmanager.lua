...
local reloadb = RLButton("Reload UI", loadf)
...
local closeb = CloseButton(tukuilocal.addon_close, loadf)
closeb:SetWidth(120)
closeb:SetHeight(22)
...
TukuiDB.CreatePanel(showb,buttontext:GetWidth()+14, 20, "TOPLEFT", UIParent, "TOPLEFT", 5, -7)
TukuiDB.CreateShadow(showb)
showb:SetFrameLevel(2)
showb:EnableMouse(true)
		
showb:HookScript("OnEnter", function(self)
	showb:SetBackdropBorderColor(unpack(TukuiCF["media"].valuecolor)) end)
showb:HookScript("OnLeave", function(self)
	showb:SetBackdropBorderColor(unpack(TukuiCF["media"].bordercolor)) end)
showb:RegisterForClicks("AnyUp")
